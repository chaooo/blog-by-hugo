...
LangChain 是一个开源的大语言模型（LLM）应用开发框架，由 Harrison Chase 于2022年底推出。它的核心定位是将大模型从单纯的文本生成和聊天能力，封装成可落地的工程化应用。你可以把它理解为“AI应用的积木套装”，通过提供标准化的组件，让开发者无需从零编写底层逻辑，就能快速搭建出智能问答、数据分析助手等复杂应用。

## 一、LangChain 的核心架构
### 1.1 核心组件
LangChain 的功能主要围绕以下六大核心模块展开：
1. **Models（模型层）**：提供统一的接口，屏蔽不同厂商 API 的差异。支持云端模型（如 OpenAI、通义千问）和本地开源模型（如 Llama），一行代码即可切换底层模型。
2. **Prompts（提示词工程层）**：标准化管理提示词，解决硬编码问题。支持模板化动态插入变量、少样本提示以及输出解析器（强制模型输出 JSON 等指定格式）。
3. **Memory（记忆层）**：为对话增加上下文存储，解决大模型多轮对话易遗忘的问题。支持完整保存历史、自动总结以节省 Token，以及向量形式的长期记忆，并可持久化到数据库或文件中。
4. **Chains（链层）**：LangChain 最核心的组件，将多个步骤按顺序串联形成工作流。例如，可以将“文档检索 → 传给大模型回答”组合成一个 RAG 核心链。
5. **Data Connection（数据连接层）**：实现大模型读取私有文档和外部数据，是构建 RAG（检索增强生成）的基础。包含文档加载器（PDF、网页等）、文本分割器、文本向量化以及各类向量数据库的对接。
6. **Agents（智能体层）**：LangChain 的高级功能，赋予 AI 自主决策的能力。智能体能够思考任务目标、规划调用顺序，并自动调用外部工具（如联网搜索、计算器、API 查询等）来完成任务。

### 1.2 典型应用场景
借助上述组件，LangChain 非常适合用于构建以下应用：
* **基于私有数据的问答系统**：结合 RAG 技术，让 AI 基于本地文档或知识库准确回答问题。
* **具备上下文的对话机器人**：利用 Memory 模块实现流畅的多轮连续对话。
* **自动化工作流与内容生成**：自动执行翻译、总结、改写等多步任务，或自动生成报告、社交媒体帖子等。
* **数据分析与代码辅助**：将自然语言转换为数据库查询语句，或进行 AI 辅助编程与代码解释。

### 1.3 LCEL（LangChain Expression Language）
LCEL 是 LangChain 的核心特性，允许你用管道符 `|` 连接组件：
```python
# 示例：提示词 → 模型 → 输出解析器
...
completion = client.chat.completions.create(
    model="deepseek-v4-pro",  # 使用模型 deepseek-v4-pro
    messages=[{"role": "user", "content": "你是谁"}]
)

print(completion.model_dump_json())
```
运行 `python test01.py`，打印回复内容:
```text
...
### 2.3 LangChain调用LLM
创建`test02.py`，LangChain调用DeepSeek：
```python
import os
import sys
from dotenv import load_dotenv
...
llm = ChatOpenAI(
    model_name="deepseek-v4-pro" # 使用模型 deepseek-v4-pro
)

response = llm.invoke("你是谁")
print(response.content)
```
运行 `python test02.py`，打印回复内容:
```text
你好！我是DeepSeek，由深度求索公司创造的AI助手。😊

我是一个纯文本模型，可以帮你解答各种问题、提供信息、进行对话交流等等。虽然我不支持多模态识别（比如直接“看懂”图片），但我可以通过文件上传功能读取图像、PDF、Word、Excel等文件中的文字信息来帮助你。

我的特点包括：
- **完全免费**使用
- **上下文长度1M**，可以一次性处理像《三体》三部曲那样体量的内容
- 支持**联网搜索**（需要手动开启）
- 支持**语音输入**（App端）
- 知识截止到2025年5月
...
让 AI 能调用外部工具（查天气、查数据库、调 API），创建`test03.py`:
```python
import os
from dotenv import load_dotenv
...
由于 `ChatPromptTemplate` 实现了标准的Runnable接口，因此可以通过链来完成调用，如下示例：
```python
from dotenv import load_dotenv
from langchain_openai import ChatOpenAI
